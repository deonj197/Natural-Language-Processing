...
namespace frmTitles
{
    public partial class Source : Form
    {
        ...
        private void Source_Load(object sender, EventArgs e)
        {
            ...
            conn.Open();
            //Connection State
            TextFind.Text = conn.State.ToString();

            //Query that you want to perform (data that is needed from the table)
            TokenWordCommand = new OleDbCommand("Select * from Source", conn);
            ...
            textDocument.DataBindings.Add("Text", TokenWordTable, "Document");
            textPOD.DataBindings.Add("Text", TokenWordTable, "POD");
            textWho.DataBindings.Add("Text", TokenWordTable, "Who");
            TextFind.DataBindings.Add("Text", TokenWordTable, "Find");
